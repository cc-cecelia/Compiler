...
import Fronted.Error.ErrorType;
...
public class Parser {
    ...
    public Stmt _Printf() {
        ...
        semiCn = lexer.now;
        lexer.next();
        if (!errorHandler.checkTypeL(formatString.getFormatString(), exps.size())) {
    ...
    public FormatString FormatString() {
        ...
        /* ERROR JUDGE BEGIN*/
        if (errorHandler.checkTypeA(strToken)) {
            errorHandler.add(new Error(ErrorType.a, strToken.lineNum));
        }
        /* ERROR JUDGE END */
        lexer.next();
        return formatString;
