...
import SymbleTable.Func.FuncParam;
import SymbleTable.Func.SymFuncType;
import SymbleTable.FuncSymbol;
import SymbleTable.Symbol;
import SymbleTable.TablesController;
import SymbleTable.VarSymbol;
...
public class Parser {
    ...
    public void match(LexType lexType) {
        if (lexer.now.type != lexType) {
            switch (lexType) {
                case SEMICN:
                    errorHandler.add(new Error(ErrorType.i, lexer.backTrack().lineNum));
                    lexer.fix(";");
                    break;
                case RPARENT:
                    errorHandler.add(new Error(ErrorType.j, lexer.backTrack().lineNum));
                    lexer.fix(")");
                    break;
                case RBRACK:
                    errorHandler.add(new Error(ErrorType.k, lexer.backTrack().lineNum));
                    lexer.fix("]");
                    break;
                case RBRACE:
                    lexer.fix("}");
                ...
                default:
                    break;
                //throw new RuntimeException("Invalid Error At " + lexer.now.lineNum);
            }
        }
    }
    ...
    public ConstDef ConstDef() {
        ...
        ident = lexer.now;
        if (errorHandler.isRedefined(ident)) {
            errorHandler.add(new Error(ErrorType.b, ident.lineNum));
        }
        lexer.next();
        // now = [ | =
    ...
    public VarDef VarDef() {
        ...
        match(LexType.IDENFR);
        ident = lexer.now;
        boolean flag = errorHandler.isRedefined(ident);
        if (flag) {
            errorHandler.add(new Error(ErrorType.b, ident.lineNum));
        }
        // now = ident
    ...
    public FuncDef FuncDef() {
        ...
        if (this.errorHandler.isRedefined(ident)) {
            errorHandler.add(new Error(ErrorType.b, ident.lineNum));
            errorHandler.ignoreScope = true;
        } else {
        ...
        // 进入下一作用域
        if (!errorHandler.ignoreScope) {
            controller.enterNewScope(funcSymbol);
            isEnterFunc = true;
            isExitFunc = true;
        }
        ...
        if (funcSymbol != null) {
            if (funcSymbol.getFuncType() == SymFuncType.INT && !block.isLastRet()) {
                errorHandler.add(new Error(ErrorType.g, block.getRBrace().lineNum));
            }
        }
        ...
        isExitFunc = false;
        errorHandler.ignoreScope = false;
...
//            symbolParam = new FuncParam(funcFParam.getDim());
//        } else if (Judge.match(preRead, LexType.LBRACE)) {
//            // 有错 但是就留给funcdef处理
//            funcFParam = new FuncFParam(bType, ident, null, null, lBracks, rBracks, constExps);
//            symbolParam = new FuncParam(funcFParam.getDim());
//        } else {
//            throw new IllegalStateException("标识符之后还能有？" + preRead);
//        }
    ...
    public Block Block() {
        ...
        // 进入新的作用域
        if (!isEnterFunc && !errorHandler.ignoreScope) {
            controller.enterNewScope(controller.getFuncSym(controller.curLevel));
        }
        isEnterFunc = false;
        ...
        // 退出作用域
        if (!isExitFunc && !errorHandler.ignoreScope) {
            controller.exitScope();
        }
    ...
    private Stmt _CtrlFlow() {
        ...
        if (!isInLoop) {
            errorHandler.add(new Error(ErrorType.m, ctrl.lineNum));
        }
    ...
    private Stmt _Return() {
        ...
        if (errorHandler.checkVoidReturn(exp == null)) {
            errorHandler.add(new Error(ErrorType.f, returnTK.lineNum));
        }
    ...
    public Stmt _Printf() {
        ...
        /* ERROR JUDGE BEGIN*/
        if (errorHandler.checkTypeA(formatString)) {
            errorHandler.add(new Error(ErrorType.a, formatString.lineNum));
        }
        /* ERROR JUDGE END */
        lexer.next();
        // now = , | )
        ...
        semiCn = lexer.now;
        lexer.next();
        if (!errorHandler.checkTypeL(formatString, exps.size())) {
            errorHandler.add(new Error(ErrorType.l, printf.lineNum));
        }
    ...
    public Stmt _LVal1() {
        ...
        if (errorHandler.constReassign(lVal.getIdent())) {
            errorHandler.add(new Error(ErrorType.h, lVal.getIdent().lineNum));
        }
    ...
    public LVal LVal() {
        ...
        if (!errorHandler.isEverExist(controller.curLevel, ident, VarSymbol.class)) {
            errorHandler.add(new Error(ErrorType.c, ident.lineNum));
        }
    ...
    public UnaryExp UnaryExp() {
        ...
        if (Judge.isUnaryOp(lexer.now)) {
            ...
            if (Judge.match(preRead, LexType.LPARENT)) {
                ident = lexer.now;
                if (!errorHandler.isEverExist(controller.curLevel, ident, FuncSymbol.class)) {
                    errorHandler.add(new Error(ErrorType.c, ident.lineNum));
                }
                lexer.next();
                match(LexType.LPARENT);
                ...
                switch (errorHandler.checkFuncRParams(ident)) {
                    case 0 -> errorHandler.add(new Error(ErrorType.d, ident.lineNum));
                    case 1 -> errorHandler.add(new Error(ErrorType.e, ident.lineNum));
                }
                controller.clear();
                lexer.next();
